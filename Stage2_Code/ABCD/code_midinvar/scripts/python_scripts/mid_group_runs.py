...
def group_onesample(fixed_files,group_out,contrast,smoothing=None):
    ...
    sec_lvl_model = SecondLevelModel(smoothing_fwhm=smoothing,mask_img=mni_mask)
...
run = sys.argv[3] # run info

#contrast list
contrasts = ["Lgain-Neut"]#
#, "LSgain-Neut", "Lgain-Lloss",
#             "Lloss-Neut", "LSloss-Neut", "Lloss-Lgain"]

#brain mask
mni_mask="/home/faird/mdemiden/analyses/code_midinvar/ROI/MNI152NLin2009cAsym_res-02_desc-brain_mask.nii.gz"
...
for con in contrasts:
    ...
    fix_maps = sorted(glob.glob(f'{fixed_input}/**/*run-{run}_contrast_{con}_zstat.nii.gz'))
