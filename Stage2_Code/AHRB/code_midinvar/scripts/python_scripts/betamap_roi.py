    ...
    for r in runs:
         for c in contrasts:
             for roi,img in rois.items():
                 beta_map = image.load_img(f'{first_input}/{sub}/{sub}_ses-01_task-mid_run-{r}_contrast_{c}_beta.nii.gz')
...
for sub in subj:
    ...
    for r in runs:
        for roiname,roi_img in rois.items():
            brain = image.load_img(f'{fmri_prep}/{sub}/ses-1/func/{sub}_ses-1_task-mid_run-{r}_*brain_mask.nii.gz')
