...
for r in runs:
    ...
    beh = pd.read_csv(f'{data_path}/{sub}/ses-1/func/{sub}_ses-1_task-mid_{r}_events.tsv', sep='\t')
    # Neutral Hit: if df['TRIAL_TYPE'] == 'NoMoneyStake' 
    beh.loc[(beh['TRIAL_TYPE'] == 'NoMoneyStake') & ((beh['PROBE_HIT'] == 1)), 'TRIAL_RESULT'] = 'Hit_No Money Stake!'
    # Neutral Miss: if df['TRIAL_TYPE'] == 'NoMoneyStake' 
    beh.loc[(beh['TRIAL_TYPE'] == 'NoMoneyStake') & ((beh['PROBE_HIT'] == 0)), 'TRIAL_RESULT'] = 'Miss_No Money Stake!'

    # get path to confounds from fmriprep & get data in dataframe filling missing values with 0 (e.g., derivatives)
    conf = glob.glob(
        f'{deriv_path}/{sub}/ses-1/func/{sub}_ses-1_task-mid_{r}_desc-confounds_timeseries.tsv'
    )[0]
    ...
    nii_path = glob.glob(
            f'{deriv_path}/{sub}/ses-1/func/{sub}_ses-1_task-mid_{r}_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz'
    )[0]
    
    # Get path to functional mask
    mask_path = glob.glob(
        f'{deriv_path}/{sub}/ses-1/func/{sub}_ses-1_task-mid_{r}_space-MNI152NLin2009cAsym_res-2_desc-brain_mask.nii.gz'
    )[0]
    ...
    design_matrix = create_designmat(events_df=beh, conf_regressors=conf_regressors,
                                     hrf_model='SPM', tr=tr, volumes=vols,stc=True)
    ...
    for con_name, con in contrasts.items():
       # zscore file
       zstat_name = f'{scratch_out}/{sub}_ses-01_task-mid_{r}_contrast_{con_name}_zstat.nii.gz'
       ...
       # Calc: beta estimate
       beta_name = f'{scratch_out}/{sub}_ses-01_task-mid_{r}_contrast_{con_name}_beta.nii.gz'
       beta_est = run_fmri_glm.compute_contrast(con, output_type='effect_size')
       beta_est.to_filename(beta_name)
       # Calc: variance
       var_name = f'{scratch_out}/{sub}_ses-01_task-mid_{r}_contrast_{con_name}_var.nii.gz'
    ...
    plot_design_matrix(design_matrix=design_matrix,
                       output_file=f'{scratch_out}/{sub}_ses-01_task-mid_{r}_model-FirstLevelDesign.png')
    ...
    nib.save(img= clean_bold_fwhm5mm,
             filename=f'{scratch_out}/{sub}_ses-01_task-mid_{r}_space-MNI152NLin2009cAsym_res-2_desc-filtered_bold.nii.gz'
             )
