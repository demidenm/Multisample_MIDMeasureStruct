...
def create_designmat(events_df, tr, volumes, conf_regressors, hrf_model='glover', stc=True):
    ...
    events_df["ANTICIPATION_DURATION"] = events_df["Cue.Duration"] + events_df["Fix.Duration"]
    ...
    onsets = pd.concat([events_df.loc[:, "Cue.OnsetTime"], events_df.loc[:, "Feedback.OnsetTime"]],
                       ignore_index=True)
    duration = pd.concat([events_df.loc[:, "ANTICIPATION_DURATION"], events_df.loc[:, "Feedback.Duration"]],
                         ignore_index=True)
...
for r in runs:
    ...
    beh = pd.read_csv(f'{data_path}/{sub}/{ses}/func/{sub}_{ses}_task-reward_{r}_events.tsv', sep='\t')
    # Neutral hit: if df['Condition'] == 'Triangle'
    beh.loc[(beh['Condition'] == 'Triangle') & ((beh['Stimulus.ACC'] == 1) | (beh['stimacc'] == 1)), 'Result'] = 'Hit_No Money Stake!'
    # Neutral miss: if df['Condition'] == 'Triangle'
    beh.loc[(beh['Condition'] == 'Triangle') & ((beh['Stimulus.ACC'] == 0) | (beh['stimacc'] == 0)), 'Result'] = 'Miss_No Money Stake!'

    # get path to confounds from fmriprep & get data in dataframe filling missing values with 0 (e.g., derivatives)
    conf = glob.glob(
        f'{deriv_path}/{sub}/{ses}/func/{sub}_{ses}_task-reward_{r}_desc-confounds_timeseries.tsv'
    )[0]
    ...
    nii_path = glob.glob(
            f'{deriv_path}/{sub}/{ses}/func/{sub}_{ses}_task-reward_{r}_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz'
    )[0]
    
    # Get path to functional mask
    mask_path = glob.glob(
        f'{deriv_path}/{sub}/{ses}/func/{sub}_{ses}_task-reward_{r}_space-MNI152NLin2009cAsym_res-2_desc-brain_mask.nii.gz'
    )[0]
    ...
    for con_name, con in contrasts.items():
       # zscore file
       zstat_name = f'{scratch_out}/{sub}_{ses}_task-reward_{r}_contrast_{con_name}_zstat.nii.gz'
       ...
       # Calc: beta estimate
       beta_name = f'{scratch_out}/{sub}_{ses}_task-reward_{r}_contrast_{con_name}_beta.nii.gz'
       beta_est = run_fmri_glm.compute_contrast(con, output_type='effect_size')
       beta_est.to_filename(beta_name)
       # Calc: variance
       var_name = f'{scratch_out}/{sub}_{ses}_task-reward_{r}_contrast_{con_name}_var.nii.gz'
    ...
    plot_design_matrix(design_matrix=design_matrix,
                       output_file=f'{scratch_out}/{sub}_{ses}_task-reward_{r}_model-FirstLevelDesign.png')
    ...
    nib.save(img= clean_bold_fwhm5mm,
             filename=f'{scratch_out}/{sub}_{ses}_task-reward_{r}_space-MNI152NLin2009cAsym_res-2_desc-filtered_bold.nii.gz'
             )
