...
runs = ["01", "02"]
task = 'reward'
ses = '1' #session value for MLS is case specific given run 1/2 v run 2/3 availability, see ses_r below
...
print("1. Mean ROI Running for: ", subj)
...
# MLS has slightly modified session info, to workaround, for sub use sess in col2, .loc[1]
sess_info = pd.read_csv('./qc_out/id_ses.tsv', delimiter='\t', header = None)

print("3.  Voxel in ROI Running for: ", subj)
for sub in subj:
    filter_dat = sess_info.iloc[:, 0] == sub
    ses_r = sess_info.loc[filter_dat, 1].values
    for r in runs:
        for roiname,roi_img in rois.items():
            brain = image.load_img(f'{fmri_prep}/{sub}/ses-{ses_r}/func/{sub}_ses-{ses_r}_task-{task}_run-{r}_*brain_mask.nii.gz')
