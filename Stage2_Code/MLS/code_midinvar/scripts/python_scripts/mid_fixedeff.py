...
def fixed_effect(sub, task_type, contrasts, input_path, output_path,
    ...
    e.g., '{sub}_{ses}_task-{task_type}_effect-fixed_contrast-{c}_stat-effect.nii.gz'
    ...
    for c in contrasts:
        ...
        if save_beta == True:
            fix_effect_out = f'{output_path}/{sub}_{ses}_task-{task_type}_effect-fixed_contrast-{c}_stat-effect.nii.gz'
            fix_effect.to_filename(fix_effect_out)
        if save_var == True:
            fix_var_out = f'{output_path}/{sub}_{ses}_task-{task_type}_effect-fixed_contrast-{c}_stat-var.nii.gz'
            fix_var.to_filename(fix_var_out)
        if save_tstat == True:
            fix_tstat_out = f'{output_path}/{sub}_{ses}_task-{task_type}_effect-fixed_contrast-{c}_stat-tstat.nii.gz'
...
fixed_effect(sub=sub, contrasts=contrasts, task_type='reward',
