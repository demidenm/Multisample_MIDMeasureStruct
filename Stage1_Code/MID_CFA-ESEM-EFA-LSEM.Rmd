...
In this section, a population model is fit reflecting the approach and avoidance (measured constructs) that are [presumed] to be associated with stimuli within/across tasks (Manifest variables). As formalized here, in the MID task the contrasts reflect the manifest variables (i.e., reflective model) as opposed of the other way, whereby the items form the variables predict the construct (i.e., formative model).
...
Randomly creating sex at birth variable. Running Multigroup CFA on this variable. In future [real data] implementation, will apply the EFA CFA from n = 1000 ABCD sample in the held out n = 1000 ABCD sample. Piloting on the simulated sex variable here.
...
Standardized Parameters in Table
```{r}
parameters(sex_cfa, standardize = T)
```
